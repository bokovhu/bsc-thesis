...
public class NodeTemplate implements Serializable {
    ...
    private static NodeTemplate createFromBuiltinTemplate(BuiltinTemplate templ) {
        return NodeTemplate.builder()
                .name(templ.name())
                .factory(templ.factory)
                .ports(templ.ports)
                .properties(templ.properties)
                .supportsChildren(templ.supportsChildren)
                .build();
    }

    private static final List<NodeTemplate> ALL = new ArrayList<>(
            Arrays.stream(BuiltinTemplate.values())
            .map(NodeTemplate::createFromBuiltinTemplate)
            .collect(Collectors.toUnmodifiableList())
    );

    public static synchronized List<NodeTemplate> getAll() {
        return Collections.unmodifiableList(ALL);
    }

    public static synchronized NodeTemplate forName(String name) {
        return ALL.stream()
    ...
    public static synchronized void register(NodeTemplate newTemplate) {
        ALL.add(newTemplate);
    }

    private final String name;
    private final boolean supportsChildren;
    private final Function<SurfaceFactoryRequest, Evaluable<Float, CPUContext, GPUContext>> factory;
    ...
    enum BuiltinTemplate {
        ...
        CONE(
                List.of(
                        Property.builder()
                                .defaultValue((float) Math.toRadians(30.0))
                                .type("float")
                                .name("angle")
                                .build(),
                        Property.builder().defaultValue(1.0f).type("float").name("height").build()
