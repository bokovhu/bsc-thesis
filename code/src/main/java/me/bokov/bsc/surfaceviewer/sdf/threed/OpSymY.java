...
public class OpSymY implements CPUEvaluator<Float, CPUContext>, GPUEvaluator<GPUContext> {
    private final Evaluable<Float, CPUContext, GPUContext> generator;

    private final Vector3f tmpP = new Vector3f();

    public OpSymY(Evaluable<Float, CPUContext, GPUContext> generator) {
        ...
        result.add(
                var(
                        ...
                        vec3(
                                ...
                                opMul(literal(-1.0f), ref(context.getPointVariable(), "y")),
