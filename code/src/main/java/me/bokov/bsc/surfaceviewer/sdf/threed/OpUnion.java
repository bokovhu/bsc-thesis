...
public class OpUnion implements PerPointSDFGenerator3D, GLSLDistanceExpression3D {
    ...
    public float getAt(float x, float y, float z) {
        return (float) Math.min(a.cpu().evaluate(new Vector3f(x, y, z)), b.cpu().evaluate(new Vector3f(x, y, z)));
    }
